    ...
    public override object ReadJson(Type type, Dictionary<string, object> values)
    {
        if (type == typeof(Color))
        {
            ...
            if (values.ContainsKey("a"))
                result = new Color(CastFloat(values["r"]) / 255f, CastFloat(values["g"]) / 255f, CastFloat(values["b"]) / 255f, CastFloat(values["a"]));
            else
                result = new Color(CastFloat(values["r"]) / 255f, CastFloat(values["g"]) / 255f, CastFloat(values["b"]) / 255f, 0f);

            return result;
        }
        else
        {
            throw new System.NotImplementedException("Can only read Color. Not objects of type " + type);
        }
    }
