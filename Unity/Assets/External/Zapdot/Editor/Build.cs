...
public class Build : MonoBehaviour
{
    ...
    private static string[] scenes = new string[] { 
                                                    "Assets/Scenes/Game_Elliott.unity", 
                                                  };
    ...
    private static void BuildCopyMusic(string buildLocation)
    {
        string sourcePath = Path.GetFullPath(Application.dataPath + "/../Music");

        DirectoryInfo dirSongsSource = new DirectoryInfo(sourcePath);

        if (!dirSongsSource.Exists)
        {
            UnityEngine.Debug.LogError("Build: Songs Directory Missing!" + sourcePath);
        }
        else
        {
            DirectoryInfo dirSongsDest = Directory.CreateDirectory(buildLocation + "Music");

            FileInfo[] files = dirSongsSource.GetFiles();
            foreach (FileInfo file in files)
                file.CopyTo(Path.Combine(dirSongsDest.FullName, file.Name), true);

            UnityEngine.Debug.Log("[BUILD] Music Copied");
        }
    }
    ...
    protected static void RunNoStoreAllProdBuild()
    {
        ...
        RunNoStoreAndroidProdBuild(AndroidBuildSubtarget.ETC);
        RunNoStoreAndroidProdBuild(AndroidBuildSubtarget.ATC);
        RunNoStoreAndroidProdBuild(AndroidBuildSubtarget.DXT);
        RunNoStoreAndroidProdBuild(AndroidBuildSubtarget.PVRTC);
    }
    ...
    protected static void RunNoStoreAndroidProdBuild(AndroidBuildSubtarget textureCompression)
    {
